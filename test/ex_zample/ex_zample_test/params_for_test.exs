defmodule ExZample.ParamsForTest do
  ...
  describe "params_for/1 when using modules" do
    import ExZample, only: [params_for: 1]

    test "uses struct default values" do
      default_map = simple_str_map(%UserWithDefaults{})

      assert default_map == params_for(UserWithDefaults)
    end

    test "uses the example values" do
      default_map = %UserWithDefaultsAndExample{} |> Map.from_struct()

      assert default_map != (user = params_for(UserWithDefaultsAndExample))
    ...
    test "uses the example/1, a.k.a full control, callback" do
      assert params_for(UserWithFullControlFactory) == %{
               "id" => 2,
    ...
    test "uses the example/0 callback when both are defined" do
      assert params_for(UserWithFullControlAndExampleFactory) == %{
               "id" => 777,
    ...
    test "recursively map the structs" do
      assert params_for(SocialUserFactory) == %{
               "user" => %{
                 "age" => 21,
                 "email" => "test@test.test",
                 "first_name" => "First Name",
    ...
    test "fails with invalid module" do
      assert_raise ArgumentError, fn -> ExZample.params_for(__MODULE__) end
    end
  end

  describe "params_for/1 when using aliases" do
    import ExZample, only: [params_for: 1]
    ...
    test "builds using the global scope when there's no scope specified" do
      assert %{"first_name" => "A name", "last_name" => "Last Name"} == params_for(:user)
    end
    ...
    test "fails with unregistered alias" do
      assert_raise ArgumentError, fn -> params_for(:unregistered_alias) end
    end
  end

  describe "params_for/2" do
    import ExZample, only: [params_for: 2]

    test "overrides given attributes on struct default values" do
      default_struct = %UserWithDefaults{}

      user = params_for(UserWithDefaults, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on example values" do
      user = params_for(UserWithDefaultsAndExample, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on structless modules" do
      user = params_for(UserFactory, first_name: "Overrided First Name", age: 28)
    ...
    test "uses the example/1, a.k.a full control, callback" do
      assert params_for(UserWithFullControlFactory, first_name: "overridden", id: 3) ==
               %{
                 "id" => 6,
    ...
    test "uses the example/1 callback when both are defined" do
      assert params_for(UserWithFullControlAndExampleFactory, last_name: "overridden", age: 3) ==
               %{
                 "id" => 2,
                 "first_name" => "full-control:first_name",
  ...
  def access_invalid_key, do: ExZample.params_for(UserFactory, company: "Test company")

  describe "params_list_for/2" do
    import ExZample, only: [params_list_for: 2]

    test "returns empty list when count is 0" do
      assert [] == params_list_for(0, UserWithDefaults)
    end
    ...
    test "builds up to given count" do
      ...
      list = params_list_for(count, UserWithDefaults)
    ...
    test "uses struct default values" do
      ...
      assert [default_map] == params_list_for(1, UserWithDefaults)
    end

    test "uses the example values" do
      default_map = simple_str_map(%UserWithDefaultsAndExample{})

      assert [user] = params_list_for(1, UserWithDefaultsAndExample)
    ...
    test "works with structless modules" do
      assert [
             ...
             ] = params_list_for(1, UserFactory)
    end
  end

  describe "params_list_for/3" do
    import ExZample, only: [params_list_for: 3]

    test "returns empty list when count is 0" do
      assert [] == params_list_for(0, UserWithDefaults, first_name: "Test")
    end

    test "builds up to given count" do
      count = Enum.random(1..100)

      list = params_list_for(count, UserWithDefaults, first_name: "Test")
    ...
    test "overrides given attributes on struct default values" do
      ...
      [user] = params_list_for(1, UserWithDefaults, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on example values" do
      [user] = params_list_for(1, UserWithDefaultsAndExample, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on structless modules" do
      [user] = params_list_for(1, UserFactory, first_name: "Overrided First Name", age: 28)
  ...
  describe "params_pair_for/1" do
    import ExZample, only: [params_pair_for: 1]

    test "uses struct default values" do
      default_map = simple_str_map(%UserWithDefaults{})

      assert {^default_map, ^default_map} = params_pair_for(UserWithDefaults)
    end

    test "uses the example values" do
      default_map = simple_str_map(%UserWithDefaultsAndExample{})

      assert {user_a, user_b} = params_pair_for(UserWithDefaultsAndExample)
    ...
    test "works with structless modules" do
      ...
      assert {^expected_example, ^expected_example} = params_pair_for(UserFactory)
    end
  end

  describe "params_pair_for/2" do
    import ExZample, only: [params_pair_for: 2]

    test "overrides given attributes on struct default values" do
      default_map = Map.from_struct(%UserWithDefaults{})

      {user_a, user_b} = params_pair_for(UserWithDefaults, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on example values" do
      {user_a, user_b} =
        params_pair_for(UserWithDefaultsAndExample, first_name: "Overrided First Name")
    ...
    test "overrides given attributes on structless modules" do
      {user_a, user_b} = params_pair_for(UserFactory, first_name: "Overrided First Name", age: 28)
